...
static const uint64_t INPUT_BUFFER_SIZE = 10 * 1024;
...
static uint64_t code_unit_to_byte(uint64_t unit) {
  return unit << 1;
}

static uint64_t byte_to_code_unit(uint64_t byte) {
  return byte >> 1;
}
...
static inline TSNode unmarshal_node(const TSTree *tree) {
  ...
  node.context[0] = code_unit_to_byte((uint64_t)TRANSFER_BUFFER[1]);
  node.context[1] = (uint64_t)TRANSFER_BUFFER[2];
  node.context[2] = code_unit_to_byte((uint64_t)TRANSFER_BUFFER[3]);
  node.context[3] = (uint64_t)TRANSFER_BUFFER[4];
  node.tree = tree;
  return node;
}
...
static inline TSTreeCursor unmarshal_cursor(const void **buffer, const TSTree *tree) {
  TSTreeCursor cursor;
  cursor.id = buffer[0];
  cursor.context[0] = (uint64_t)buffer[1];
  cursor.context[1] = (uint64_t)buffer[2];
  cursor.tree = tree;
  return cursor;
}
...
static TSPoint unmarshal_point(const void **address) {
  TSPoint point;
  point.row = (uint64_t)address[0];
  point.column = code_unit_to_byte((uint64_t)address[1]);
  return point;
}
...
static TSInputEdit unmarshal_edit() {
  ...
  edit.start_byte = code_unit_to_byte((uint64_t)*address); address += 1;
  edit.old_end_byte = code_unit_to_byte((uint64_t)*address); address += 1;
  edit.new_end_byte = code_unit_to_byte((uint64_t)*address); address += 1;
  return edit;
}
...
const uint64_t ts_tree_cursor_current_node_id_wasm(const TSTree *tree) {
  TSTreeCursor cursor = unmarshal_cursor(TRANSFER_BUFFER, tree);
  TSNode node = ts_tree_cursor_current_node(&cursor);
...
uint64_t ts_tree_cursor_start_index_wasm(const TSTree *tree) {
  TSTreeCursor cursor = unmarshal_cursor(TRANSFER_BUFFER, tree);
  TSNode node = ts_tree_cursor_current_node(&cursor);
  return byte_to_code_unit(ts_node_start_byte(node));
}

uint64_t ts_tree_cursor_end_index_wasm(const TSTree *tree) {
  TSTreeCursor cursor = unmarshal_cursor(TRANSFER_BUFFER, tree);
  TSNode node = ts_tree_cursor_current_node(&cursor);
  return byte_to_code_unit(ts_node_end_byte(node));
}

uint64_t ts_tree_cursor_current_field_id_wasm(const TSTree *tree) {
  TSTreeCursor cursor = unmarshal_cursor(TRANSFER_BUFFER, tree);
...
const char *ts_node_field_name_for_child_wasm(const TSTree *tree, uint64_t index) {
  TSNode node = unmarshal_node(tree);
...
uint64_t ts_node_child_count_wasm(const TSTree *tree) {
  TSNode node = unmarshal_node(tree);
  return ts_node_child_count(node);
}

uint64_t ts_node_named_child_count_wasm(const TSTree *tree) {
  TSNode node = unmarshal_node(tree);
  return ts_node_named_child_count(node);
}

void ts_node_child_wasm(const TSTree *tree, uint64_t index) {
  TSNode node = unmarshal_node(tree);
  marshal_node(TRANSFER_BUFFER, ts_node_child(node, index));
}

void ts_node_named_child_wasm(const TSTree *tree, uint64_t index) {
  TSNode node = unmarshal_node(tree);
  marshal_node(TRANSFER_BUFFER, ts_node_named_child(node, index));
}

void ts_node_child_by_field_id_wasm(const TSTree *tree, uint64_t field_id) {
  TSNode node = unmarshal_node(tree);
...
void ts_node_descendant_for_index_wasm(const TSTree *tree) {
  ...
  uint64_t start = code_unit_to_byte((uint64_t)address[0]);
  uint64_t end = code_unit_to_byte((uint64_t)address[1]);
...
uint64_t ts_node_start_index_wasm(const TSTree *tree) {
  TSNode node = unmarshal_node(tree);
  return byte_to_code_unit(ts_node_start_byte(node));
}

uint64_t ts_node_end_index_wasm(const TSTree *tree) {
  TSNode node = unmarshal_node(tree);
  return byte_to_code_unit(ts_node_end_byte(node));
}
...
void ts_node_children_wasm(const TSTree *tree) {
  TSNode node = unmarshal_node(tree);
  uint64_t count = ts_node_child_count(node);
...
void ts_node_named_children_wasm(const TSTree *tree) {
  TSNode node = unmarshal_node(tree);
  uint64_t count = ts_node_named_child_count(node);
  const void **result = NULL;
  if (count > 0) {
    result = calloc(sizeof(void *), 5 * count);
    const void **address = result;
...
bool symbols_contain(const uint64_t *set, uint64_t length, uint64_t value) {
...
void ts_node_descendants_of_type_wasm(
  ...
  if (end_point.row == 0 && end_point.column == 0) {
    end_point = (TSPoint) {UINT64_MAX, UINT64_MAX};
  }
...
void ts_query_matches_wasm(
  ...
  while (ts_query_cursor_next_match(scratch_query_cursor, &match)) {
    ...
    result.contents[index++] = (const void *)(uint64_t)match.pattern_index;
    result.contents[index++] = (const void *)(uint64_t)match.capture_count;
...
void ts_query_captures_wasm(
  ...
  while (ts_query_cursor_next_capture(
    ...
    result.contents[index++] = (const void *)(uint64_t)match.pattern_index;
    result.contents[index++] = (const void *)(uint64_t)match.capture_count;
    result.contents[index++] = (const void *)(uint64_t)capture_index;
    for (unsigned i = 0; i < match.capture_count; i++) {
      const TSQueryCapture *capture = &match.captures[i];
      result.contents[index++] = (const void *)capture->index;
      marshal_node(result.contents + index, capture->node);
